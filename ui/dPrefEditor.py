...
class DamnPrefEditor(wx.Dialog): # Preference dialog (not manager)
	...
	def buildModulePanel(self, parent, module, extended=False, buttons=True, withscrollbars=False):
		...
		if extended:
			...
			if mod['author'].has_key('url'):
				authorsizer.Add(self.getLabel(tmppanel, DV.l(mod['author']['name'], warn=False), bold=True, hyperlink=mod['author']['url']))
			else:
				authorsizer.Add(self.getLabel(tmppanel, DV.l(mod['author']['name'], warn=False), bold=True))
			if mod['author'].has_key('email'):
				authorsizer.Add(self.getLabel(tmppanel, DV.l(' <'), color='#707070'))
				authorsizer.Add(self.getLabel(tmppanel, DV.l(mod['author']['email'], warn=False), hyperlink='mailto:' + mod['author']['email']))
				authorsizer.Add(self.getLabel(tmppanel, DV.l('>'), color='#707070'))
			desc = self.getLabel(tmppanel, DV.l(mod['about']['long'], warn=False))
		else:
			desc = self.getLabel(tmppanel, DV.l(mod['about']['short'], warn=False))
		desc.Wrap(descwidth)
	...
	def onClose(self, event):
		DV.prefs = DamnPrefs() # Reload from ini
		self.Close(True)
