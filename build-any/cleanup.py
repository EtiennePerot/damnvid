...
def cleanDir(d, deletables):
    ...
    for i in deletables:
        if i.find('*') != -1:
            ...
            shazarr(d + os.sep + i)
cleanDir('.', ['COPYING', 'DamnVid.exe.manifest', 'required-files.txt', 'damnvid.spec', 'package', 'usr', 'DamnVid.app', '*.module.damnvid', '*.so', '*.so.*', 'build.tar.gz', 'library.zip', 'py', 'NSIS-win32.nsi', 'DamnVid', '*.tmp'])
